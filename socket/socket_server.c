...
void send_to_mpi(int powmin, int powmax) {
    char command[256];
    snprintf(command, sizeof(command), "kubectl exec -it jogodavida-mpi-deployment -- mpirun -np 4 ./jogodavidampi %d %d", powmin, powmax);
    printf("Executando comando MPI: %s\n", command);
...
void send_to_omp(int powmin, int powmax) {
    char command[256];
    snprintf(command, sizeof(command), "kubectl exec -it jogodavida-mpi-deployment -- ./jogodavidaomp %d %d", powmin, powmax);
    printf("Executando comando OpenMP: %s\n", command);
    int ret = system(command);
    if (ret == -1) {
